...
先以 ROOT 产品合同、原子需求描述和祖先合同确定明确的控件、行为与结果，再用具体场景补充准备状态、输入和边界。抽象指代或重复模板不是界面文案，也不要求额外操作。`exactUiStrings` 是从引号中提取的原词，可能是控件名、数据值、状态值或示例；必须回到原文判断用途，不能仅凭该数组认定某个字符串是 UI 名称。
...
- 每个 case 必须提供 `expectationBasis`：1 到 3 条必须逐字引用需求证据原文的引用（原样复制，不做改写），说明该 case 的终末断言所期待的结果由哪句 ROOT 产品合同、需求、场景、祖先描述、exactUiStrings、种子条目或前置需求支撑。
...
## Locator 陷阱
纯 text locator 的程序化字面依据限于 `exactUiStrings`、顶层 `seedData` 条目或本 case 先前的 fill 值；内联 `seedDeclarations` 的散文即使提到某个值，也不自动豁免同一目标的 role 或 label fallback。优先结构化 role；上述字面依据也不证明文本一定显示在页面上，应按需求确认可见性，避免让猜测的裸 text 成为唯一定位方式。
...
使用 role、label 或 text，字符串取证据中确认为相应控件名或可见结果的原值。字符串按字面匹配，忽略大小写，除非 exact 为 true，否则作为子串匹配；绝不使用正则表达式语法、alternation 或通配符。button、link、checkbox、heading、alert 优先用 role 加 name；表单控件优先用 label；保留应用声明的语言，不要翻译 label。需求原文明确给出的控件名、动作名与入口名使用 exact: true 精确匹配；数据值和示例不能被当作控件名。
...
## Locator 选择补充
需求未给出控件名称时，优先使用不带猜测 name 的结构化 role，例如主工作区用 main，唯一输入框用 textbox；即使 exactUiStrings 非空，也不能把其中的数据值当作控件名。要求展示首页的需求描述的是页面状态，而不是字面文本 Home 或一个 Home 按钮：导航到 / 并断言所需区域可见。绝不把描述性词汇变成必需的 UI label，也不臆造 seed 记录。
