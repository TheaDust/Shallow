...
4. 对于 seed data：先按原文检查初始状态，断言声明的条目在应有的位置逐字出现，再建立场景前置条件，最后执行目标操作和断言。active/可删除/未选中等描述是动作前的状态；恢复类场景需要先通过需求及前置需求声明的操作进入已归档、已删除或已选中的状态。把初始状态、准备操作、目标操作和结果放在同一个 case 中，不能因为 GIVEN 从某个状态开始，就假定种子已经变成该状态。`Seed data:` 声明的实体（含 seedDeclarations 字段列出的条目）是动作开始前就存在的初始数据，不是本场景要创建或修改的对象：即使需求描述的是创建流程，也要先断言种子实体逐字出现，再执行创建操作；创建或编辑需要新名称时，使用与任何种子名称都不同的独立值，避免与初始断言混淆。
