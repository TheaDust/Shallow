...
4. 对于 seed data：先按原文检查初始状态，断言声明的条目在应有的位置逐字出现，再建立场景前置条件，最后执行目标操作和断言。active/可删除/未选中等描述是动作前的状态；恢复类场景需要先通过需求及前置需求声明的操作进入已归档、已删除或已选中的状态。把初始状态、准备操作、目标操作和结果放在同一个 case 中，不能因为 GIVEN 从某个状态开始，就假定种子已经变成该状态。
