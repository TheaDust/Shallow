...
- 冻结字段（改动任意一个即判整个 refinement 无效）：packetId、case 数量与顺序、每个 case 的 id / requirementIds / purpose / expectationBasis、每个 case 的 step 数量、每个 step 的 op，以及除 locator 外的一切字段——goto 的 path、fill / select / expectValue 的 value、press 的 key、expectText 的 text / exact / anyOf、expectCount 的 count、终末 assertion 的 op 与全部内容。
