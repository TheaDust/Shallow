...
主线网关恢复由 `src/gateway-recovery.ts` 管理：Planner 单路排队，与 Builder 并行；两者遇到临时网关错误后共享退避窗口。控制器从 30 秒开始指数退避，常规延迟上限 5 分钟，并遵守更长的 `Retry-After`；每个逻辑调用至多重试 5 次，退避计划不得越过 15 分钟恢复窗口或阶段/Builder 调用剩余额度。每 30 秒输出等待状态。请求本身仍受原有调用超时约束。认证和请求格式错误不重试；Planner 单独的认证/协议错误保持 Judge 故障隔离。

中途写出的应用通过可运行检查后可保存为空需求列表的检查点，供原工作包继续实现；它不增加 implemented，也不授予 verified。恢复耗尽时，`pendingRequirementIds` 明确列出未完成项，父目录仅在全部后代实现后标记实现完成。
