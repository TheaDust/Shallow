...
| 会话 | 实现阶段每个工作包使用全新会话；集中修复使用新会话 | 一次运行复用同一个会话 |
...
2. `featureGroupPackets` 把原子需求确定性组成有界功能组：种子取声明序中第一个依赖已调度的项，优先扩展同一直接父目录、其次同一 ROOT 子树的可安排项，按依赖亲和与声明序 tie-break；每包最多 6 条原子需求、12 个场景、18,000 字符，单条超限独立成组。分组验证唯一覆盖与依赖顺序并落账统计；实现会话按工作包全新开始，跨包交接经代码、测试与 ARCHITECTURE.md。
